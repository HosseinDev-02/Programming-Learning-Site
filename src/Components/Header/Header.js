                    ...
                    <nav className='flex items-center justify-between relative'>
                        ...
                        <div className='flex items-center gap-3 lg:gap-8'>
                            ...
                            <ul className='hidden lg:flex items-center gap-5 font-YekanBakh-SemiBold text-sm child-hover:text-title child:transition-colors'>
                                ...
                                {
                                    menus.map(menu => (
                                        <li key={menu.menu_id} className='group/categories'>
                                            <a className='flex items-center gap-1' href={menu.link}>
                                                <span className=''>{menu.title}</span>
                                                {
                                                    menu.submenus.length ? (
                                                        <svg
                                                            className='w-5 h-5 group-hover/categories:rotate-180 transition-all'>
                                                    ...
                                                    <ul className='invisible shadow opacity-0 group-hover/categories:visible group-hover/categories:opacity-100 absolute top-full bg-background border border-border w-56 flex flex-col gap-1 child:leading-5 delay-75 transition-all shadow-black/5 text-title z-20'>
                                                        {
                                                            menu.submenus.map(submenu => (
                                                                <li key={submenu.submenu_id} className='group/subcategories'>
                                                                    ...
                                                                    {/* header categories submenu */}
                                                                    {
                                                                        submenu.menulinks.length ? (
                                                                            <ul className='invisible shadow opacity-0 group-hover/subcategories:visible group-hover/subcategories:opacity-100 transition-all delay-75 flex flex-col flex-wrap space-y-3 px-3 pt-8 pb-3 bg-background border border-border w-96 absolute right-full -top-px -bottom-px text-caption text-sm'>
                                                                                ...
                                                                                {
                                                                                    submenu.menulinks.map(menulink => (
                                                                                        <li key={menulink.menulink_id} className='w-1/2'>
                                ...
                                {/*        <a href='/dashboard' className='hover:text-primary py-2 px-3'>داشبورد</a>*/}
                        ...
                        <div className='flex items-center gap-3 md:gap-5'>
                            ...
                            <div className='group/profile'>
                                ...
                                <div id='user-profile'
                                    ...
                                    <a className='hover:text-red-700 text-red-500 flex items-center gap-2 py-2 px-3'
                                       href="#">
                                        <span>
                                            <svg className='w-5 h-5'>
                                                <use href='#logout'></use>
            ...
            <div style={mobileMenuShow ? {right: '0'} : {}} id='mobile-menu'
                ...
                <label
                    ...
                    <div
                        ...
                        <span
                            style={darkMode === 'dark' ? {transform: 'translateX(26px)', backgroundColor: '#000'} : {}}
                            className='w-3 h-3 bg-zinc-200 absolute left-0 translate-x-[2px] rounded-full top-0 bottom-0 my-auto transition-all'></span>
                    </div>
                </label>
                {/*  Mobile Menu Categories  */}
                <div>
                    <div style={showMobileCategoryMenu ? {color: 'rgb(var(--color-title))'} : {}}
                         onClick={() => setShowMobileCategoryMenu(prevState => !prevState)}
                        ...
                        <span>
                                        <svg style={showMobileCategoryMenu ? {transform: 'rotate(180deg)'} : {}}
                                             className='w-5 h-5'>
                                            <use href='#chevron-down'></use>
                                        </svg>
                                    </span>
                    </div>
                    <ul style={showMobileCategoryMenu ? {display: 'inline-block'} : {}}
                        className='child:py-2 hidden relative before:absolute before:content-[""] before:top-0 before:bottom-0 before:right-3 before:bg-zinc-200 dark:before:bg-zinc-900 before:h-full before:w-px pr-8 mt-4 child:text-xs text-zinc-400'>
                        <li>
                            <a style={showMobileCategorySubMenu ? {color: 'rgb(var(--color-title))'} : {}}
                               onClick={() => setShowMobileCategorySubMenu(prevState => !prevState)}
                               className='flex items-center gap-1'
                               href="#">
                                <svg style={showMobileCategorySubMenu ? {transform: 'rotate(-45deg)'} : {}}
                                     className='w-4 h-4'>
                                    <use href='#chevron-left'></use>
                                </svg>
                                برنامه نویسی وب
                            </a>
                            <ul style={showMobileCategorySubMenu ? {display: 'inline-block'} : {}}
